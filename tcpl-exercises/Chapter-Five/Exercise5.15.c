 ...
 * 		 The assumption is that the command will always be run as one of the following:
 *
 * 		 	./sort -n -r -f
 * 		 	./sort -n -r
 * 		 	./sort -n -f
...
int str_cmp_fold(const char *s, const char *t);
int rev_str_cmp_fold(const char *s, const char *t);
int tolower(int);
...
int main(int argc, char *argv[])
{
	...
	else if (argc > 2 && str_cmp(argv[1], "-r") == 0 && str_cmp(argv[2], "-f") == 0)
	{
		reverse = 1;
		folded = 1;
	}
        ...
        reverse = 1;
    else if (argc > 1 && str_cmp(argv[1], "-f") == 0)
    	folded = 1;
    ...
    if ((nlines = readlines(lineptr, MAXLINES)) >= 0)
    {
        ...
        else if (reverse && folded)
			quicksort((void**) lineptr, 0, nlines-1, (int (*)(void*, void*))(rev_str_cmp_fold));
        else if (numeric)
            quicksort((void**) lineptr, 0, nlines-1, (int (*)(void*, void*))(num_cmp));
        else if (reverse)
            quicksort((void**) lineptr, 0, nlines-1, (int (*)(void*, void*))(rev_str_cmp));
        else if (folded)
			quicksort((void**) lineptr, 0, nlines-1, (int (*)(void*, void*))(str_cmp_fold));
        else
...
/* str_cmp: return <0 if s<t, 0 if s==t, >0 if s>t. Upper and lowercase are treated the same */
int str_cmp_fold(const char *s, const char *t)
{
...
/* rev_str_cmp: return <0 if s>t, 0 if s==t, >0 if s<t. Upper and lowercase are treated the same */
int rev_str_cmp_fold(const char *s, const char *t)
{
	for ( ; tolower(*s) == tolower(*t); s++, t++)
		if (*s == '\0')
